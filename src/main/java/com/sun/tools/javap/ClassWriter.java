...
import static com.sun.tools.classfile.AccessFlags.ACC_VARARGS;
...
public class ClassWriter extends BasicWriter {
    ...
    public void write(ClassFile cf) {
        ...
        if (sigAttr == null) {
            ...
            try {
                ...
                // FieldType and a ClassSignatureType that only contains a
                // superclass type.
    ...
    protected void writeField(Field f) {
        ...
        if (options.showConstants && !options.compat) { // BUG 4111861 print
                                                        // static final field
                                                        // contents
    ...
    private Instruction.KindVisitor<Void, Integer> constructInstrPrinter() {
        return new Instruction.KindVisitor<Void, Integer>() {

            @Override
            public Void visitNoOperands(Instruction instr, Integer p) {
                System.out.print("{}");
                return null;
            }

            @Override
            public Void visitBranch(Instruction instr, int offset, Integer p) {
                System.out.print("{}");
                return null;
            }

            @Override
            public Void visitConstantPoolRef(Instruction instr, int index, Integer p) {
                try {
...
//                }
    ...
    protected void writeJsonMethod(Method m) {
        ...
        if (c_attr != null) {
            if (c_attr instanceof Code_attribute)
                ...
                throw new RuntimeException("Unexpected or invalid value for Code attribute");
        }
        ...
        Gson gson = new Gson();
        List<Instruction> instrsList = new ArrayList<>();

        for (Instruction instr : code.getInstructions())
            instrsList.add(instr);

        for (int i = 0; i < instrsList.size(); i++) {
            Instruction instr = instrsList.get(i);
            System.out.print("{\"op\":\"");
            System.out.print(instr.getMnemonic());

            System.out.print("\",\"params\": ");
            instr.accept(constructInstrPrinter(), 1); // ???
            System.out.print("}");

            if (i < instrsList.size() - 1) System.out.print(",");
            System.out.println("");
        }
    ...
    protected void writeMethod(Method m) {
        ...
        if (e_attr != null) { // if there are generic exceptions, there must be
                              // erased exceptions
     ...
     * 
     * @param d
     *            the descriptor, giving the expected type of the constant
     * @param index
